...
def get_all_upstreams(genes,whole_seq,b_up=400,b_down=0,is_circ=True):
    ...
    if is_circ:
        ...
        out=[Upstream(up[2],up[1],up[0],get_subseq(up,whole_seq,b_up=b_up,b_down=b_down),up[3]) for up in upstr]
    return out
...
usage='''python upstream_getter.py GENBANK bases_up bases_down IS_CIRCULAR'''
...
if __name__ == '__main__':
    ...
    for replicon in SeqIO.parse(open(genbank), 'genbank'):
        print replicon.id
